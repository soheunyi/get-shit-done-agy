...
You are invoked by:
- `/new-project` workflow (unified project initialization)
...
Your ROADMAP.md is consumed by `/plan` which uses it to:
...
You are roadmapping for ONE person (the user) and ONE implementer (the AI).
...
Read:
- SPEC.md content (core value, constraints)
...
### Step 7: Write Files
**Write files first, then return.** This ensures artifacts persist.
