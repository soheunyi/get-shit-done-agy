...
$files = Get-ChildItem -Recurse -Path .agent, .gsd\templates -Include *.md, *.txt, *.json
...
find .agent .gsd/templates -type f \( -name "*.md" -o -name "*.txt" -o -name "*.json" \) -print0 | while IFS= read -r -d '' file; do
...
if grep -rE "Claude|Anthropic|Sonnet" .agent .gsd/templates > /dev/null; then
    echo "❌ Sanitization errors found. Aborting sync."
    grep -rE "Claude|Anthropic|Sonnet" .agent .gsd/templates
    exit 1
fi
